...
static PGconn *new_dbconnection(void)
{
	...
	res = c_prepare_pg_statement::get_result_of_statement_preparations(
...
{
	available_head = reinterpret_cast<c_dbconnection *>(calloc(DBCONN_POOL_SIZE, sizeof(c_dbconnection)));
	...
	available_tail = available_head + DBCONN_POOL_SIZE - 1;

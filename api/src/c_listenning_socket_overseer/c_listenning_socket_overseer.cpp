...
/*                                                                            */
/*                                                                            */
/*   Filename: c_listenning_socket_overseer.cpp                               */
/*   Author:   Peru Riezu <riezumunozperu@gmail.com>                          */
/*   github:   https://github.com/priezu-m                                    */
/*   Licence:  GPLv3                                                          */
/*   Created:  2024/08/03 03:49:19                                            */
/*   Updated:  2024/08/06 01:48:20                                            */
/*                                                                            */
/* ************************************************************************** */

#include "c_listenning_socket_overseer.hpp"
...
c_listenning_socket_overseer::c_listenning_socket_overseer(void)
{
	...
	if (bind(listening_sock, reinterpret_cast<struct sockaddr *>(&server_addr), slen) == -1)
	{
		std::cerr << std::string("PRIORITY=3\n") + "SYSLOG_FACILITY=3\n" + "SYSLOG_IDENTIFIER=" +
						 c_worker_id_to_text::get_name_from_id(worker_id) +
						 "\nMESSAGE=" + "failed to bind listening socket to the file system: " + internal_strerror(errno);
		exit(EXIT_FAILURE); // NOLINT(concurrency-mt-unsafe)
	}
	...
	g_listenning_socket_overseer = this;
}

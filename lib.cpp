...
char const     *unimplemented_content =
...
char const *http_404_content = "HTTP/1.0 404 Not Found\r\n"
							   "Content-type: text/html\r\n"
							   "\r\n"
							   "<html>"
							   "<head>"
 ...
 */
void fatal_error(char const *syscall)
{
	perror(syscall);
	_exit(1);
}
...
void send_static_string_content(char const *str, int client_socket)
{
...
char const *get_filename_ext(char const *filename)
{
...
void send_headers(char const *path, off_t len, struct iovec *iov) // NOLINT
{
	 ...
	 * */
	char const *file_ext = get_filename_ext(small_case_path);
...
int get_line(char const *src, char *dest, int dest_sz)
{
